...
def load_users():
    if not os.path.exists(USERS_FILE):
        # Create default users file with IDs
        default_users = [
            {'id': 1, 'username': 'admin', 'password': 'admin123', 'role': 'admin'},
            {'id': 2, 'username': 'user', 'password': 'user123', 'role': 'user'}
        ]
    ...
    with open(USERS_FILE, 'r') as f:
        users = json.load(f)
        # Migrate dict format to list with IDs if needed
        if isinstance(users, dict):
            migrated = []
            next_id = 1
            for username, info in users.items():
                migrated.append({'id': next_id, 'username': username, 'password': info['password'], 'role': info['role']})
                next_id += 1
            with open(USERS_FILE, 'w') as fw:
                json.dump(migrated, fw, indent=2)
            return migrated
        return users
...
def load_sell_history():
    if not os.path.exists(SELL_HISTORY_FILE):
        return []
    with open(SELL_HISTORY_FILE, 'r') as f:
        return json.load(f)
...
class InventoryApp:
    ...
    def main_screen(self):
        ...
        file_menu.add_command(label='Sell Item', command=self.sell_item)
        ...
        # Reports menu
        reports_menu = tk.Menu(self.menu, tearoff=0)
        reports_menu.add_command(label='Stock Report', command=self.stock_report)
        reports_menu.add_command(label='Sales Report', command=self.sales_report)
        reports_menu.add_command(label='Customer Report', command=self.customer_report)
        reports_menu.add_command(label='Summary Report', command=self.summary_report)
        self.menu.add_cascade(label='Reports', menu=reports_menu)
        ...
        columns = ('ID', 'Name', 'Quantity', 'Price', 'Total Price', 'Cost Price', 'Sell')
        ...
        self.tree.column('Sell', width=80, anchor='center')
        self.tree.pack(side='left', fill='both', expand=True)
        scrollbar = ttk.Scrollbar(table_frame, orient='vertical', command=self.tree.yview)
        scrollbar.pack(side='right', fill='y')
        self.tree.configure(yscrollcommand=scrollbar.set)
        self.refresh_list()
        # Add sell button for each item
        def on_tree_click(event):
            item_id = self.tree.identify_row(event.y)
            col = self.tree.identify_column(event.x)
            if col == '#6' and item_id:
    ...
    def refresh_list(self):
        ...
        if hasattr(self, 'tree'):
            ...
            for idx, item in enumerate(self.inventory, 1):
                ...
                self.tree.insert('', 'end', values=(idx, item['name'], item['quantity'], item['price'], total_price, item.get('cost_price', 0), 'Sell'))
        self.update_grand_total()
    ...
    def add_item(self):
        ...
        def submit():
            ...
            if not name:
                ...
                return
            self.inventory.append({'name': name, 'quantity': quantity, 'price': price, 'cost_price': cost_price})
    ...
    def edit_item(self):
        ...
        def submit():
            ...
            if not name:
                messagebox.showerror('Error', 'Name cannot be empty!')
                return
            self.inventory[idx] = {'name': name, 'quantity': quantity, 'price': price, 'cost_price': cost_price}
            save_inventory(self.inventory)
            self.refresh_list()
    ...
    def view_history(self):
        ...
        columns = ('No.', 'Name', 'Quantity Sold', 'Price', 'Total Sale', 'Discount (%)', 'Discount Price', 'Final Total', 'Customer Name', 'Contact Number', 'Timestamp')
        tree = ttk.Treeview(history_dialog, columns=columns, show='headings', height=10)
        tree.heading('No.', text='No.')
        tree.heading('Name', text='Name')
        tree.heading('Quantity Sold', text='Quantity Sold')
        tree.heading('Price', text='Price')
        tree.heading('Total Sale', text='Total Sale')
        tree.heading('Discount (%)', text='Discount (%)')
        tree.heading('Discount Price', text='Discount Price')
        tree.heading('Final Total', text='Final Total')
        ...
        tree.column('Price', width=80, anchor='center')
        tree.column('Total Sale', width=100, anchor='center')
        tree.column('Discount (%)', width=80, anchor='center')
        tree.column('Discount Price', width=100, anchor='center')
        tree.column('Final Total', width=100, anchor='center')
        tree.column('Customer Name', width=120, anchor='center')
        ...
        scrollbar.pack(side='right', fill='y')
        tree.configure(yscrollcommand=scrollbar.set)
        ttk.Button(history_dialog, text='Close', command=history_dialog.destroy, style='Inventory.TButton').pack(pady=10)
